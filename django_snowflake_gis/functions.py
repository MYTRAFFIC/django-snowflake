from django.contrib.gis.db.models.functions import AsGeoJSON, Centroid, Distance, Length, Perimeter
...
def snowflake_centroid(self, compiler, connection, **extra_context):
    source_expressions = self.get_source_expressions()
    clone = self.copy()
    clone.set_source_expressions(source_expressions[:1])

    function = connection.ops.spatial_function_name("ST_CENTROID")

    return super(Centroid, clone).as_sql(
        compiler, connection, function=function, **extra_context
    )
...
def register_functions():
    ...
    Centroid.as_snowflake = snowflake_centroid
